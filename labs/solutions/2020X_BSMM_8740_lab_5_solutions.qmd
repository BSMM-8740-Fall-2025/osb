...
# use the training results, and find points on the curve for a given threshold
build_roc_auc <- function(threshold, dat = training_results){
  # threshold on predictions
  log_preds <- ifelse(dat$.pred_Yes > threshold, 1, 0)
  # compute predictions and reference/truth | turn both into factors
...
**(ii)** Use the **assignment** variable to plot the cluster assigments generated by `stats::kmeans`
