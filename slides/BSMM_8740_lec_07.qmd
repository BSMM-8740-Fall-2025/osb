...
::: {style="font-size: x-large"}
-   In an AR(1) process the present value is a linear function of the previous observation. $$y_t=\phi_1 y_{t-1}+\epsilon_t$$
-   Given the starting value $y_0=0$, and repeated substitution, this process could be represented as $$y_t=\sum_{j=1}^t\phi_1^{t-j}\epsilon_j$$
-   Could include several lags (i.e. AR(p) model) and the distribution of the error term could take many forms
:::
