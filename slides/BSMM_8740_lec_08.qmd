...
#| echo: false
#| eval: false
ip_wts <- propensity_model |>
  broom::augment(newdata = dat_, type.predict = "response") |> 
  dplyr::mutate(
    ipw = dplyr::case_when(n == 1 ~ 1/.fitted, TRUE ~ 1/(1-.fitted))
    , ipw_alt = (net / .fitted) + ((1 - net) / (1 - .fitted) )
    , mean_exposure = mean(net)
  )
  # dplyr::select(id, net, ipw = .fitted) |> 
  # dplyr::mutate('_ipw' = 1/(1-ipw), ipw = 1/ipw)

hist(ip_wts$ipw, breaks = 50)
hist(net_data_wts$wts, breaks = 50)

net_data_wts$wts

ip_wts |> dplyr::summarize(max = max(ipw))
...
Stabilized weights will are often used to address this, e.g. the function `propensity::wt_ate` multiplies the weights by the mean of the treatment so $SW^A=\mathbb{E}[D]/\mathbb{E}[D|L]$.
...
We know that IPW weighting should create a pseudo-population that make the covariates more balanced by treatment. Let's check this:
