...
Last we we used IPW to create a pseudopopulation where, for every confounder level, the numbers of treated and untreated were balanced.
...
We know that we can also predict the effect of treatment by building a model to predict the outcome by using, for example, a regression model
...
Average treatment effect on the treated (TT[^1]) gives us summary stats on the sample or the population.

[^1]: You'll sometimes see this written as ATT
...
## DAGs - more advanced confounding
...
The `tip_coef()` function takes an estimate (e.g. the upper or lower bound of the coefficient) and further requires either the - the scaled differences in means of the confounder between exposure groups or - the effect of the confounder on the outcome.
