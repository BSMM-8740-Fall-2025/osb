...
#| echo: true
#| code-fold: true
#| code-summary: integrate the (un-noramlized) half-normal
...
The property of Markov Chains is memoryless, because we only need to remember what square we've reached, not which squares we visited to get here.
...
We can illustrate $\mathsf P$ by a transition diagram/graph, where the nodes/vertices are the states and the arrows give the transition probabilities. (We don't draw the arrow if $p_{ij} = 0$.) In this case, our transition diagram looks like this:
:::
