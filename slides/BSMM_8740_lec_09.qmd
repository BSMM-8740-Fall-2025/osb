...
1.  draw iid $x_1,x_2,\ldots,x_n$ from $f$ and calculate the importance weight
...
1.  Draw $x_1,x_2,\ldots,x_n$ from $f(x)$ and calculate importance weights $w(x_i)=g(x_i)/f(x_i)$
2.  $\int h(x)g(x)dx\approx\frac{1}{M}\sum_{i=1}^Mh(x_i)g(x_i)/f(x_i)$\
3.  $\int g(x)dx\approx\frac{1}{M}\sum_{i=1}^Mg(x_i)/f(x_i)$\
4.  Estimate $\mathbb{E}_f\left[h(X)g(X)/f(X)\right]$ by $$
