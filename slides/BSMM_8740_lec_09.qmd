...
Random numbers from a given distribution are drawn using random numbers from a ($[0,1]$) uniform distribution, using the **inverse transform sampling method**. Here’s how it works:
...
What defines a good random number generator (RNG) is a long period – how long it takes before the sequence repeats itself. A period of $2^{32}$ is not enough (need at least $2^{40}$). Various statistical tests exisit to measure “randomness” – well validated software will have gone through these checks.

## Random Number Generation

Alternatives to inverse transform sampling are special algorithms that generate random samples from a particular distributions, e.g. one way to sample from a Gaussian random number generation uses the Box-Muller method:
...
Monte Carlo methods avoid doing any integration or summation and instead just generate lots of samples of $Z$, say $z_1,z_2,\ldots,z_M$ and *estimate* $\mathbb{E}[h(Z)]$ as $\frac{1}{M}\sum_{i=1}^Mh(z_i)$. The law of large numbers states that this sample mean should be close to $\mathbb{E}[h(Z)]$.
...
#| echo: true
#| code-fold: true
#| code-summary: calculation of some kind
...
[^1]: The **geometric distribution** is a discrete pmf that can be interpreted as the number of failures before the first success ($\mathbb{P}(X=k)=(1-p)^{k-1}p$, with mean $p$).
...
The key to calculating this is to *condition on the first step* again -- that is, on whether the customer has an account on Tuesday. We have
...
::: {style="font-size: xx-large"}
If we start from a state given by a pmf $\boldsymbol X_0=\pi = (\pi_i)_{i\in S}$, then after step 1 the probability we're in state $j$ is $\sum_i \pi_i p_{ij}$.

Given transition matrix $\mathsf P$ and initial state pmf $X_0$, the pmf for the state after one step is $X_1=X_0 \times\mathsf P$, i.e. $X_0$ and $X_1$ are row vectors, and for any given state pmf, the pmf for the states on the next step of the Markov chain is the current state pmf multiplied by the transition matrix (note: on the left).
:::
...
-   ::: {style="font-size: xx-large"}
    A Markov Chain is [**irreducible**]{.underline} if you have positive probability of eventually getting in the state space from anywhere to anywhere else in the state space.
...
This version is called [**random-walk Metropolis**]{.underline} - if we always accepted the propsals, they would form a traditional random walk process.
:::

## MCMC Metropolis Algo

-   the burn-in period is a matter fo trial and error
...
For metropolis, th key was that the Markov chain was reversible, i.e. $\pi(x)\mathsf{P}(x,y)=\pi(y)\mathsf{P}(y,x)$
