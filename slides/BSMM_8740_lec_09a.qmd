...
::: {style="font-size: x-large"}
Suppose that $q(x)>0;\;x\in D$ and $\int_Dq(x)dx=Z_q<\infty$. Here $q()$ is an un-normalized density on $D$ whereas the corresponding normalized density is $\frac{1}{Z_q}q(x)$.
...
A Markov Chain is **aperiodic** if there are no forced cycles, i.e. there do not exist disjoint non-empty subsets $X_1,X_2,...,X_d$ for $d≥2$,such that $P(x,Xi+1)=1$ for all $x\in Xi (1≤i≤d−1)$,and $P(x,X1)=1$ for all $x\in Xd$.
:::
