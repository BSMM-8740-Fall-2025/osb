...
sim <- function() {
  ...
  return(list(att = att, ipw = ipw_estimate, regadjust = ra_estimate, ols = ols_estimate))
}
...
bias_data <- data.frame(
  ...
  bias = c(results_df$ols_bias, results_df$regadjust_bias, results_df$ipw_bias),
  method = factor(rep(c("OLS", "Reg. Adjust", "IPW"), each = nrow(results_df)))
