...
```{r data-prep}
#| echo: true
movie_scores <- fivethirtyeight::fandango %>%
  dplyr::rename(
...
parsnip::linear_reg() %>%
  parsnip::set_engine("lm") # lm: linear model
```
...
parsnip::linear_reg() %>%
  ...
  parsnip::fit(audience ~ critics, data = movie_scores)
```
...
new_movie <- tibble::tibble(critics = 50)

# predict the outcome for a new movie
movie_fit %>% 
  stats::predict(new_movie)
```
