...
twfe_model |> broom::tidy() |> dplyr::mutate(model = 'twfe_plm_model', .before = 1) |> 
  dplyr::bind_rows(
...
ggplot(plot_data, aes(x = time, y = mean_y, group = group, color = group)) +
...
ggplot(df, aes(x = time, y = y, group = unit, 
               color = factor(ifelse(unit <= 10, "Control", "Treated")))) +
  geom_line(alpha = 0.3,  ) +
  geom_point() +
