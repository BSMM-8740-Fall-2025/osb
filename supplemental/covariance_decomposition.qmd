...
# Parameters
n_units <- 20        # Number of units/entities
...
df <- tibble::tibble(
  ...
  , treatment =
    dplyr::case_when(unit >= 10 ~ 1, TRUE ~ 0)
...
twfe_model |> broom::tidy() |> dplyr::mutate(model = 'twfe_model', .before = 1) |> 
  dplyr::bind_rows(
...
df |> 
  dplyr::select(time:y) |> 
  tidyr::pivot_wider(values_from = treatment:y)

df |> 
  dplyr::select(unit:y) |> 
  tidyr::pivot_wider(names_from=unit,values_from = time:y)
